<template>
  ...
  <div>
    <div class="flex justify-content-center">
      <div>
        <h3 style="margin-bottom: 15px">Regulamin</h3>
        <div style="width: 90vw; background-color: white; border-radius: 10px;">
          <span v-if="userRole == 'ADMIN'">
            <Editor v-model="miscellaneous.rules" editorStyle="height: 600px" />
          </span>
          <span v-else>
            <div v-html="miscellaneous.rules" style="height: 600px; padding: 20px"></div>
          </span>
        </div>
      </div>
    </div>
