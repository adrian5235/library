<template>
  ...
  <div class="flex justify-content-center">
    <div>
      <h3 style="margin-bottom: 15px;">Pozostałe wydania książki {{ book.title }}</h3>
      <div class="card">
        <DataView :value="editions" layout="grid" paginator :rows="6" :rowsPerPageOptions="[6, 12, 18, 24]" style="width: 90vw">
          <template #header>
            <div class="flex align-items-center justify-content-center gap-2">
              <div class="flex align-items-center gap-2">
                ...
                <Dropdown v-model="selectedLanguage" :options="languages" optionLabel="name" showClear filter @change="filter()" placeholder="Wybierz język" class="w-full md:w-14rem" />
                <Dropdown v-model="selectedBinding" :options="bindings" optionLabel="name" showClear filter @change="filter()" placeholder="Wybierz oprawę" class="w-full md:w-14rem" />
              </div>
              <div class="align-items-center">
                <span class="p-input-icon-left">
                  <i class="pi pi-search" />
                    <InputText v-model="searchValue" placeholder="Numer ISBN" @keyup="filter()" />
                </span>
              </div>
            </div>
          </template>
          ...
          <template #empty>
            <span style="margin-left: 5px">Nie znaleziono innych wydań</span>
          </template>
              ...
              <div class="p-4 border-1 surface-border surface-card border-round">
                <div class="flex flex-wrap align-items-center justify-content-between gap-2">
                  <span class="overflow-hidden white-space-nowrap text-overflow-ellipsis" style="max-width: 70%">{{ slotProps.data.language.name }}</span>
                    <Tag v-if="slotProps.data.quantity > 0" value="dostępne" severity="success"></Tag>
                    <Tag v-else value="niedostepne" severity="danger"></Tag>
                </div>
